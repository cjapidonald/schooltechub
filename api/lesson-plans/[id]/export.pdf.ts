...
export default async function handler(request: Request): Promise<Response> {
  ...
  try {
    pdfBuffer = await renderLessonPlanPdf(exportData.plan);
  } catch (error) {
    console.error("Failed to generate lesson plan PDF", error);
    return errorResponse(500, "Failed to generate PDF export");
  }

  const fileName = createLessonPlanExportFileName(exportData.plan, "pdf");
